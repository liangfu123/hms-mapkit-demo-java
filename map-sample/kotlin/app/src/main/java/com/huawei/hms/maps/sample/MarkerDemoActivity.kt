...
class MarkerDemoActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    fun addMarker(view: View?) {
        if (mParis == null && mOrsay == null && mSerris == null) {
            ...
            hMap.setOnMarkerClickListener { marker ->
                ...
                Toast.makeText(applicationContext, clusterable.toString(), Toast.LENGTH_SHORT).show()
                true
            }
        }
    ...
    fun setTag(view: View?) {
        ...
        if (mParis != null && tagStr.isNotEmpty()) {
            mParis?.tag = tagStr
        }
    }
    ...
    fun setSnippet(view: View?) {
        ...
        if (mOrsay != null && snippetStr != null && "" != snippetStr) {
    ...
    fun setTitle(view: View?) {
        ...
        if (mParis != null && titleStr != null && "" != titleStr) {
            mParis?.title = titleStr
            Toast.makeText(this, "BJ title is " + mParis?.title, Toast.LENGTH_SHORT).show()
        }
    }
}